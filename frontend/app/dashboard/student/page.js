...
export default function StudentDashboard() {
    ...
    return (
        <div className="space-y-12 pb-20 max-w-7xl mx-auto px-6">
            ...
            <header className="relative p-12 rounded-[3rem] bg-secondary-action/5 border border-white/5 overflow-hidden group">
                <div className="absolute top-0 right-0 w-96 h-96 bg-secondary-action/10 blur-[100px] -mr-48 -mt-48 rounded-full" />
                ...
                <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-12">
                    <div className="space-y-6">
                        ...
                        >
                            <Sparkles className="w-3.5 h-3.5 text-secondary-action" />
                            <span className="text-[10px] font-black text-secondary-action uppercase tracking-[0.3em]">Phase 1 Verification Complete</span>
                        </motion.div>
                        <h1 className="text-5xl md:text-6xl font-display font-black text-white tracking-tight leading-none">
                            Greetings, <span className="text-secondary-action font-black">{user.fullName.split(' ')[0]}.</span>
                        </h1>
                        <p className="text-surface-light/40 font-medium max-w-md italic border-l-2 border-secondary-action/30 pl-6 text-lg">
                            "The pursuit of knowledge is the blueprint for future excellence."
                        </p>
                    </div>

                    <div className="flex items-center gap-12">
                        <div className="text-right group-hover:translate-x-[-10px] transition-transform duration-700">
                            <p className="text-[10px] font-black text-surface-light/20 uppercase tracking-[0.3em] mb-4">Academic Tier</p>
                            <p className="text-3xl font-display font-black text-white flex items-center justify-end gap-3">
                                <Trophy className="w-6 h-6 text-secondary-action" />
                                {stats?.tier || 'Tier A'}
                            </p>
                        </div>
                        <div className="w-[1px] h-16 bg-white/10 hidden md:block" />
                        <div className="text-right group-hover:translate-x-[-10px] transition-transform duration-700 delay-100">
                            <p className="text-[10px] font-black text-surface-light/20 uppercase tracking-[0.3em] mb-4">Aggregate Accuracy</p>
                            <p className="text-3xl font-display font-black text-white">{stats?.aggregateScore || '84.2%'}</p>
                        </div>
                    </div>
                </div>
            </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {[
                    ...
                    <motion.div
                        ...
                        className="glass-card p-10 group hover:border-white/10 transition-all duration-700 relative overflow-hidden"
                    >
                        ...
                        <div className="flex items-center justify-between mb-10">
                            <div className={`p-4 rounded-2xl bg-white/5 border border-white/5 ${stat.color} group-hover:scale-110 transition-all duration-500 shadow-2xl`}>
                        ...
                        <div className="flex items-baseline gap-3">
                            ...
                            {stat.sub && <span className="text-[10px] font-black text-surface-light/10 uppercase tracking-widest">{stat.sub}</span>}
                        </div>
                        {stat.progress && (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                        ...
                        <div className="grid gap-8">
                            {myCourses.slice(0, 4).map((course, idx) => (
                                <motion.div
                                    ...
                                    className="group glass-card p-8 flex flex-col md:flex-row items-center gap-10 hover:border-secondary-action/20 transition-all duration-700 relative overflow-hidden"
                                >
                                    ...
                                    <div className="w-full md:w-40 h-28 rounded-3xl bg-white/5 overflow-hidden shrink-0 border border-white/10 group-hover:border-secondary-action/30 transition-all duration-700 relative">
                                    ...
                                    <div className="flex-1 min-w-0 space-y-5">
                                        <div className="flex items-center justify-between">
                                            <div>
                                                <h4 className="text-xl font-black text-white truncate group-hover:text-secondary-action transition-colors mb-1">{course.title}</h4>
                                                <p className="text-[10px] font-black text-surface-light/20 uppercase tracking-widest italic">{course.instructor?.fullName || 'Senior Faculty'}</p>
                                            </div>
                                            <span className="text-[10px] font-black text-secondary-action uppercase tracking-widest bg-secondary-action/10 px-3 py-1 rounded-lg">
                                    ...
                                    <Link
                                        ...
                                        className="w-16 h-16 bg-secondary-action/5 rounded-[1.5rem] flex items-center justify-center text-secondary-action hover:bg-secondary-action hover:text-white transition-all shadow-2xl group-hover:scale-110"
                                    >
                ...
                <div className="lg:col-span-4 space-y-10">
                    ...
                    <div className="glass-card p-10 space-y-10 relative overflow-hidden group border border-white/5">
                        ...
                        {liveClasses.length > 0 ? liveClasses.map((item, i) => (
                            <div key={i} className="flex gap-8 relative group/item">
                                <div className="flex flex-col items-center gap-3">
                                    <span className="text-[10px] font-black text-surface-light/20 w-20 uppercase tracking-widest pt-1">
                                        {new Date(item.scheduledTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                    </span>
                                </div>
                                <div className="space-y-3 relative pb-10 border-l border-white/5 pl-8 ml-[-1px]">
                                    ...
                                    <h5 className="text-base font-black text-white group-hover/item:text-secondary-action transition-colors leading-tight">{item.title}</h5>
                                    ...
                                    <button
                                        ...
                                        className="mt-4 text-[9px] font-black text-secondary-action uppercase tracking-[0.3em] flex items-center gap-2 group-hover/item:gap-4 transition-all"
                                    >
